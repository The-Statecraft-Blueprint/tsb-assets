...
- **Trend line:** the chart's oxblood line is an 11-year centered moving average of the House party-unity-votes series (`house_trend_10yr_centered` in data.csv) — i.e., the true annual-data equivalent of the "~10-year" window Legislative Gridlock (`book/obs01-unresolved-agenda`) uses via its 5-Congress centered average, so the two companion charts now describe their trend windows the same way. A shortened window is used at the series' endpoints (1969, 2025). It is computed from, but not a substitute for, the raw annual series, which remains the primary line on the chart. The moving average bridges the 2023 data gap using the surrounding years. (This window was widened from an initial 5-year average — see Corrections.)
...
- **The 1974 reference line has been removed from this chart** (as of July 2026 — see Corrections). The marker was built for the earlier essay (#107, "It Begins With Why?"), which had already named the FECA/transparency-reform argument; this chart now supports Chapter 2 (observation), which hasn't made that argument yet, so the marker was visually importing a case this chapter doesn't assert. This is a chapter-wide fix, not specific to this chart — the same marker was stripped from every other Ch. 2 observation chart that had one (Gridlock, Legislation Decline, Moderate Decline, Congressional Approval). The Convergence synthesis chart intentionally keeps its 1974 indexing, since that chart is where the reform argument is actually made. (Secondary, chart-specific note: on this series 1974 also sits in a local dip — 29.4%, lower than both 1971 and 1973 — so the line wouldn't have lined up with anything visible here even if kept. That's supporting color, not the main reason.)
...
| Date | Change |
|------|--------|
| 2026-07-17 | (1) Removed the 1974 reference-line marker. It was built for the earlier essay (#107) which had already named the FECA/transparency-reform argument; this chart now supports Chapter 2 (observation), which hasn't made that argument yet, so any surviving marker visually imported an unmade case. This is a chapter-wide correction — the same marker was also removed from the Legislation Decline, Moderate Decline, and Congressional Approval charts (Gridlock already had it removed previously). Secondary/supporting note: on this data 1974 also sits in a local dip (29.4%, lower than 1971 and 1973), so it wouldn't have lined up with anything visible here regardless. (2) Added a 5-year centered moving-average trend line (oxblood) alongside the raw jagged series (navy, unchanged) to make the underlying direction legible given genuine year-to-year swings of 15-35 points; added `house_trend_5yr_centered` to data.csv. (3) Added the 2024 House party-unity-votes data point (65.3%, already present in data.csv and sourced from the CQ Roll Call 2024 study cited above) to the plotted series — it had been omitted, leaving the chart jumping straight from 2022 to 2025. (4) Corrected the "avg. member unity score in 1970" callout from 74% to 72%: the 74% figure had actually been computed from 1971's House dem/gop scores (72, 76) rather than 1970's (71, 72, average 71.5% → 72%). (5) Documented 2023 explicitly as a genuine House party-unity-votes data gap (no source publishes this figure for that year) rather than leaving it silently absent; the chart now shows a visible break in the raw line at 2023, bridged by the trend line. |
| 2026-07-17 | Widened the trend line from a 5-year to an 11-year centered moving average, for consistency with the companion Legislative Gridlock chart (`book/obs01-unresolved-agenda`), whose 5-Congress centered average is ~10 calendar years. Eleven years is the annual-data equivalent of that window. Renamed `house_trend_5yr_centered` to `house_trend_10yr_centered` in data.csv and recomputed all values; updated the chart legend and tooltip label from "5-year moving average" / "5-yr average" to "10-yr average"; updated the Methodology Notes trend-line bullet accordingly. The raw annual series (navy) and all callout statistics are unaffected. |
...
| July 2026 | Widened trend line from 5-year to 11-year (~10-year) centered moving average, matching Legislative Gridlock's window convention (full detail in Corrections above) | TSB |
